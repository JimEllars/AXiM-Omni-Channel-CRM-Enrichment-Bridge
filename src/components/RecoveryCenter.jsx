...
import { FiAlertCircle, FiRefreshCw, FiTrash2, FiEdit3, FiSearch, FiCheck, FiX } from 'react-icons/fi';
...
export default function RecoveryCenter({ onRetrySuccess }) {
  ...
  useEffect(() => {
    const saved = storage.get('dlq_items', [
      { id: 'ERR-992', source: 'LinkedIn_Scraper', reason: 'Invalid Email Syntax', payload: '{"name": "John", "email": "john.at.domain", "company": "Acme"}', time: '5m ago' },
      { id: 'ERR-993', source: 'B2B_Portal', reason: 'Field Mapping Error', payload: '{"company": "AXiM", "contact": null}', time: '12m ago' }
    ]);
    setItems(saved);
  }, []);

  const saveDLQ = (newItems) => {
    setItems(newItems);
    storage.set('dlq_items', newItems);
  };
  ...
  const handleUpdate = () => {
    const updated = items.map(i => i.id === editingItem.id ? { ...i, payload: editPayload } : i);
    saveDLQ(updated);
    setEditingItem(null);
  };
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => { setEditingItem(item); setEditPayload(item.payload); }} className="p-2 bg-slate-800 hover:bg-blue-600/20 text-slate-400 hover:text-blue-400 rounded-lg transition-colors">
                        <SafeIcon icon={FiEdit3} />
                      </button>
                      <button onClick={() => handleRetry(item)} className="p-2 bg-slate-800 hover:bg-emerald-600/20 text-slate-400 hover:text-emerald-400 rounded-lg transition-colors">
                        <SafeIcon icon={FiRefreshCw} />
                      </button>
                      <button onClick={() => saveDLQ(items.filter(i => i.id !== item.id))} className="p-2 bg-slate-800 hover:bg-red-600/20 text-slate-400 hover:text-red-400 rounded-lg transition-colors">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl">
            ...
            <div className="p-6">
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button onClick={handleUpdate} className="px-6 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold flex items-center gap-2">
